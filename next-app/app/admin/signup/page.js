      ...
      <section className="relative z-10 flex-1 flex items-center justify-center px-6">
        <div className="w-full max-w-md liquid-glass p-8 rounded-3xl animate-fade-rise">
          <div className="mb-8 text-center">
            <h1 className="text-4xl text-white mb-2" style={{ fontFamily: "var(--font-display)" }}>Teacher Portal</h1>
            <p className="text-sm text-muted-foreground">Register to manage your classroom</p>
          </div>

          <form onSubmit={handleSignup} className="space-y-4">
            <div>
              <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Full Name</label>
              <input type="text" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-2xl py-3 px-4 text-white mt-1" />
            </div>
            <div>
            ...
            <div className="text-center pt-4">
              <Link href="/admin/login" className="text-sm text-muted-foreground hover:text-white">Already registered? Login</Link>
            </div>
          </form>
        </div>
      </section>
